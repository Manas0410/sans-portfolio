...
export default function Contact() {
  ...
  const rows = [
    ...
    { icon: 'github', label: 'GitHub', value: '@' + contact.socials.github.split('/').pop(), href: contact.socials.github },
    { icon: 'linkedin', label: 'LinkedIn', value: 'in/' + contact.socials.linkedin.split('/').pop(), href: contact.socials.linkedin },
  ]

  return (
    <section id="contact" ref={ref} className="relative overflow-hidden py-32 px-6">
      ...
      <footer className="relative mx-auto mt-28 max-w-7xl border-t border-white/8 pt-8">
        <div className="flex flex-col items-center justify-between gap-4 text-sm text-mist md:flex-row">
          <span>
            © {new Date().getFullYear()} {meta.name}. Designed & built with care.
          </span>
          <span className="font-mono text-xs">
            {meta.location} · {meta.available ? 'Available for work' : 'Currently booked'}
          </span>
        </div>
      </footer>
    </section>
